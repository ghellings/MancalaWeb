...
sub fetch :Local {
    my ( $self, $c ) = @_;
    my $gamedata = $c->model('Games')->fetch_game_hash($c->req->body_params->{'gameid'} );
    $c->stash( game => $gamedata );
}
...
sub checkmove :Local {
    ...
    my $game = $c->model('Games')->fetch_game_obj($gameid);
    my $ug = Data::UUID->new;
    my $res = $ug->compare($player,$game->player->gameid);
    if ( $res ) {
        my $valid_input = $game->board->move($house,$player);
        if ( $valid_input ) {
            return $game;
        }
        $c->error("Not a valid move");    
    }
    else {
        $c->error("It is not playerid $player 's turn");
    }    
}
